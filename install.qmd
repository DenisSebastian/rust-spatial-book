...
Para verificar si has instalado Rust correctamente, abre una shell y escribe esta
línea:
...
Ahora que has instalado Rust, es hora de escribir tu primer programa. Crea un
archivo llamado `main.rs` (la extensión `.rs` identifica archivos Rust) con el
siguiente contenido:
...
`println!` es una **macro** (nota el `!`) que escribe texto en la terminal.
`fn main()` es el punto de entrada de todo programa Rust: es la primera función
que se ejecuta.

En el resto del libro usaremos **Cargo** para gestionar proyectos, lo que
simplifica la compilación y el manejo de dependencias.
...
`cargo check` acelerará el proceso de informarte si tu proyecto todavía está
