...
Las variables en Rust son inmutables por defecto — declarar `let x = 5` crea un valor que no puede cambiar, a diferencia de R donde toda asignación es mutable. Rust distingue enteros (`i32`, `u64`), flotantes (`f64`), booleanos y caracteres con tipos fijos, lo que previene coerciones silenciosas como las de R. Para análisis espacial, los tipos más frecuentes son `f64` (coordenadas) y `String`/`&str` (nombres de capas o campos). En el siguiente capítulo veremos cómo controlar el flujo de ejecución con `if`, `loop` y `match`.

## Reto Práctico

En R, calculas el área de un círculo dado su radio así:
...
Traduce este código a Rust. Declara `radio` como una variable `f64`, usa la constante `PI` de la biblioteca estándar (`std::f64::consts::PI`) y usa `println!` para mostrar el resultado con dos decimales.
...
fn main() {
    ...
    println!("Área: {:.2}", area); // Imprime: Área: 78.54
}
```

Nota que en Rust usamos `.powi(2)` para elevar al cuadrado un número entero, o `.powf(2.0)` si el exponente es flotante. El especificador de formato `{:.2}` controla la cantidad de decimales al imprimir.
:::
