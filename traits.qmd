...
- Si haces `plot(mis_datos_geoespaciales)`, R busca internamente `plot.sf` (si usas el paquete `sf`).
...
Los traits definen comportamiento que distintos tipos pueden compartir — equivalente a interfaces o métodos genéricos S3/S4 de R, pero verificados en tiempo de compilación. Un tipo implementa un trait con `impl MiTrait for MiStruct`. `#[derive(Debug, Clone, PartialEq)]` es la forma más común de obtener comportamiento estándar sin escribirlo a mano. En GeoRust, los traits como `Centroid`, `Area` y `Contains` permiten que el mismo código funcione con `Point`, `Polygon` o cualquier geometría. En el siguiente capítulo veremos los bloques **`impl`**, donde se escribe el comportamiento concreto.

## Reto Práctico

Define un trait `Descripcion` con un método `describir(&self) -> String`. Impleméntalo para un struct `Coordenada { lat: f64, lon: f64 }` de forma que retorne `"lat: X, lon: Y"`.
...
impl Descripcion for Coordenada {
    fn describir(&self) -> String {
        format!("lat: {:.4}, lon: {:.4}", self.lat, self.lon)
    }
}

fn main() {
    let santiago = Coordenada { lat: -33.4489, lon: -70.6693 };
    println!("{}", santiago.describir());
    // lat: -33.4489, lon: -70.6693
}
```

El trait solo declara la firma; el `impl` proporciona el cuerpo concreto. Si tienes múltiples tipos (Ciudad, Aeropuerto, Puerto) todos pueden implementar `Descripcion` y pasarse a la misma función que acepte `&impl Descripcion`.
:::
